...
Magnetic = new function() {
	...
	function loop() {
		...
		for( j = 0, jlen = magnets.length; j < jlen; j++ ) {
			...
			if( magnet.dragging ) {
				console.log(mouseX, mouseY, magnet.position.x, magnet.position.y)
				magnet.position.x += ( mouseX - magnet.position.x ) * 0.02;
				magnet.position.y += ( mouseY - magnet.position.y ) * 0.02;
			}
		...
		for (i = 0, ilen = particles.length; i < ilen; i++) {
			...
			dy = cs.position.y - p.position.y
			if (dy > cs.size*3) {
				if (Math.abs(p.position.x - cs.position.x) < p.orbit*.2) {
					p.fillColor = 'red'
					p.shift.y += 1
				}
			} else if (-dy > cs.size*3) {
				if (Math.abs(p.position.x - cs.position.x) < p.orbit*.2) {
					p.fillColor = 'red'
					p.shift.x += 2*p.side
					p.shift.y -= 2
			...
			particle.position.x = particle.shift.x + Math.cos(i + particle.angle) * (particle.orbit*particle.force*particle.side*-1);
			particle.position.y = particle.shift.y + Math.sin(i + particle.angle) * (particle.orbit*particle.force);
			
			// Limit to screen bounds
			if (particle.side == 1)
				particle.position.x = Math.max( Math.min( particle.position.x, SCREEN_WIDTH), closestMagnet.position.x, 0);
			else
				particle.position.x = Math.max( Math.min( particle.position.x, SCREEN_WIDTH, closestMagnet.position.x), 0 );
